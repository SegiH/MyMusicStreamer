	 ...
	 if ( count($files) == 2 && $root . $_POST['dir'] <> $music_path ) {
		  ...
		  foreach( $files as $file ) {
			   if( file_exists($root . $_POST['dir'] . $file) && $file != '.' && is_dir($root . $_POST['dir'] . $file) ) {		
			        if ( ($root . $_POST['dir'] . $file) <> $music_path . "..") { // If we are in the root dir ($music_path) do not add .. directory
                         ...
                         if ( $file != ".." || ( $file == ".." && $hasFiles == FALSE)) {
					          echo "<li class=\"directory collapsed\"><a href=\"#\" rel=\"" . htmlentities($_POST['dir'] . $file) . $delimiter . "\">" . htmlentities($file) . "</a></li>";
				         } elseif ( $hasFiles == TRUE) {
				              echo "<li class=\"directory collapsed\" ><a href=\"#\" rel=\"" . htmlentities($_POST['dir'] . $file) . $delimiter . "\">" . htmlentities($file) . "<div class='actions'><a class='addAllTracks' rel='" . urlencode($_POST['dir']) . "'>add all</a></div></a></li>";
					     }
				    }
			   }
		  }
		  ...
		  foreach( $files as $file ) {
		       if( file_exists($root . $_POST['dir'] . $file) && $file != '.' && $file != '..' && !is_dir($root . $_POST['dir'] . $file) && in_array(strtolower(substr($file,-3)),array( 'mp3','m4a','aac'))) {
				    ...
				    if ( $_POST["Type"] <> "Dir") {
					     echo "<li class='track'><a href=\"#\" rel=\"" . htmlentities($_POST['dir'] . $file) . "\">" . htmlentities($file) . "<div class='actions'><a class='add' rel='" . urlencode($_POST['dir'] . $file) . "'>add</a></div></li>";
				    }
			   }
		  }
